...
public class RMLMDPSimulator extends RMLSimulator {
	...
	public RMLMDPSimulator(RMLModel rmlModel) {
		super(rmlModel);
	}
	...
	public InterfaceState newPath(long seed){
		...
		schedulerSeed = seed;
		schedulerRng = new Random(schedulerSeed);
		// We doesn't initialize the actionRng with the seed but use 
		// the seed to initialize the scheduler (get a known scheduler)
		actionRng = new Random(mainRng.nextLong());
		initialState = rmlModel.createInitialState(model.getDefaultSystem(),0.0,schedulerRng);
