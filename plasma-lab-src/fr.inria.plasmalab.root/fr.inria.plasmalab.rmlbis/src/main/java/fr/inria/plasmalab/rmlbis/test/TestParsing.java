...
public class TestParsing {
	...
	public static boolean testFile(String filename) {
		try {
			PRISMParserProcessing parser = new PRISMParserProcessing(new File(filename));
	...
	public static void browseDirectory(String dirname) {
    	File dir = new File(dirname);
		if (!dir.getName().startsWith(".")) { 	// ignore hidden files or directories
			if (dir.isFile()) {
				if (dir.getName().endsWith(".nm") || dir.getName().endsWith(".pm") || dir.getName().endsWith(".sm")) {
