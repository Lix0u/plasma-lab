 ...
 *
 */
public class ModelConverter implements IStringConverter<Model> {
    
    private static final String separator_ = ":";

    @Override
    public Model convert(String modelString) {
        String[] modelArray = modelString.split(separator_);
        if (modelArray.length != 2){
            throw new ParameterException("The model description must follow this pattern file:type");
        }
        return new Model(modelArray[0], modelArray[1]);
    }
}
